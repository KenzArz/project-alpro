...
const string kartuATM[4] = {"BCA", "BRI", "BTN", "BNI"}; //kartu atm yang tersedia di EL-KBMN
const int penarikanDana[4] = {50000, 100000, 500000, 1000000};

// function tipe data void;
void head(); // memanggil header
void cekSaldo();
...
bool checkSandi(int kartu, int count) {
    ...
    if(!isValidSandi && count < 3) {
        ...
        system("pause"); //system akan berhenti sampai nasabah memencet sembarang tombol
        return checkSandi(kartu, count); // fungsi akan memanggil dirinya sendir, dengan nilai count yang sudah ditambah 1
    }
